...
def info_colpisci():
	...
	for indice_colonna in range(int(high)):
		for indice_cella in range(int(width)):
		...
		#se non è stata ancora colpita la cella la si colpisce
				...
				response1 = requests.post(url1, json = {"x": indice_cella, "y": indice_colonna})
				print(response1.json())
				sleep(3)

				response = requests.get(url)
				campo2 = response.json()["field"]["grid"]

				y_random = random.randint(0, high)
				x_random = random.randint(0, width)

				colonna_corrente2 = campo2[y_random]
				cella_corrente = colonna_corrente2[x_random]
